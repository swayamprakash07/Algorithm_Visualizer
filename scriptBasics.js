...
const modalclear = document.getElementById("clearbtntext");
...
const dropDownspeed = document.getElementsByClassName(
  "speeddropdownContent"
)[0];
const dropDownmaze = document.getElementsByClassName("mazaDropDowncontent")[0];
const dropDownclear = document.getElementsByClassName(
...
window.onload = () => {
  ...
  for (i = 0; i < HEIGHT; i += 1) {
    ...
    a.style.borderLeft = "1px solid rgb(175, 216, 248)";
  }
  for (i = 0; i < WIDTH; i += 1) {
    let id = HEIGHT - 1 + "-" + i;
    let a = document.getElementById(id);
...
gridTable.onmousedown = (event) => {
  ...
  if (
    !mouseclicked &&
    !(
      event.target.className.includes("start") ||
      event.target.className.includes("end")
    )
  )
    mouseclicked = true;
  else {
    mouseclicked = false;
    if (event.target.className == "start") startDrag = true;
    else endDrag = true;
  }
};
...
gridTable.onmouseup = () => {
  ...
  else if (startDrag || endDrag) startDrag = false;
  endDrag = false;
};
    ...
    for (var k = 0; k < WIDTH; k++) {
      if (nodeNo == 655) {
        cR +=
          ...
          "-" +
          k +
          "' draggable='false' onclick='createWall(event)' class='start'></td>";
      } else if (nodeNo == 678) {
        cR +=
          "<td id='" +
          i +
          "-" +
          k +
          "' draggable='false' onclick='createWall(event)' class='end' style=''></td>";
      } else {
        cR +=
          "<td id='" +
          i +
          "-" +
          k +
          "' draggable='false' onclick='createWall(event)' class='unv'></td>";
      }
      nodeNo++;
    }
    cR += "</tr>";
...
function createWall(event) {
  ...
  if (
    !cn.id.includes("row") &&
    !(cn.className.includes("start") || cn.className.includes("end"))
  ) {
...
// Change Clear
function changeClear(element) {
  modalclear.textContent = element.textContent;
  dropDownmaze.style.display = "none";
}
...
function DropDown(event) {
  ...
  if (event.target == modalclear && dropDownclear.style.display == "block") {
    dropDownclear.style.display = "none";
  } else if (
    event.target == modalclear &&
...
document.getElementById("btnVis").onmousedown = () => {
  !visualizing
    ? (document.getElementById("btnVis").style.backgroundColor =
        "rgb(30, 223, 165)")
    : undefined;
};
...
function getNodeById(id) {
  for (var i in ListOfNodes) {
    if (ListOfNodes[i].id == id) return ListOfNodes[i];
  }
}
//update the distance
...
function findMinDistanceInGraph(graph) {
  ...
  let min = Infinity;
  while (len-- != 0) {
    if (graph[len].distance < min) min = graph[len].distance;
  }
